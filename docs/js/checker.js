...
const DrawingChecker = (() => {
  ...
  const COMMON_CHECKS = [
    ...
    },
    {
      ...
      label: '充電設備設置場所の入口',
      description: '充電設備設置場所への入口が全て記載されているか（▼マーク・「出入口」テキスト等）',
      required: true,
    },
    {
      id: 'charging_space',
...
- 施設全体の敷地形状
- 敷地内における充電スペースの位置
${type === 'mokutekichi' ? '- 案内板設置位置\n- 施設に接する公道と他の施設との位置関係' : '- 施設周辺の建物等との位置関係'}
...
` : ''}### ③充電設備設置場所の入口
- ${type === 'mokutekichi' ? '公道からの' : ''}充電設備設置場所への入口を**全て**記載
...
${type === 'mokutekichi' ? '- 公道は敷地外に描画され、公道名がテキストで記載\n' : ''}- 周辺建物も描画して位置関係を表現
...
${type === 'mokutekichi' ? '- **公道名称のテキスト**を確認（道路上に記載されている）\n- **案内板の情報**（サイズ・設置方法・新設/既設・位置・向き）を注意深く読み取る\n- 案内板情報は青色テキストで記載されていることが多い' : ''}
...
- 「設置場所見取図」の文字が正確に表記されているかを厳密にチェック`;
  ...
  async function verifyModel(apiKey, modelId) {
    ...
    try {
      ...
      if (e.name === 'AbortError') {
        return { available: false, reason: 'タイムアウト' };
      }
  ...
  async function callGemini(apiKey, images, type, modelId) {
    ...
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 300_000); // 5分

    let response;
    try {
      response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(useModel)}:generateContent?key=${encodeURIComponent(apiKey)}`,
        {
          method: 'POST',
      ...
      if (e.name === 'AbortError') {
        throw new Error('Gemini APIへの接続がタイムアウトしました（5分）。ネットワーク状況を確認して再試行してください。');
      }
  ...
  function aggregateResults(geminiResult, type) {
    ...
    const VALID_STATUSES = ['pass', 'fail', 'warn'];
    const items = checks.map(check => {
      const result = resultMap[check.id] || { status: 'fail', found_text: '', detail: '判定結果が取得できませんでした' };
      // 想定外のステータスは要確認(warn)にフォールバック
      const status = VALID_STATUSES.includes(result.status) ? result.status : 'warn';
      return {
        ...check,
        status,
    ...
    // - fail 3件以上: 不合格
    let overall;
    if (requiredFail === 0 && requiredWarn === 0) {
      overall = 'pass';
